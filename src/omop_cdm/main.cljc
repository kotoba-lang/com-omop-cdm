(ns omop_cdm.main
  "Kotodama WASM entrypoint for the Omop_cdm clean-room actor (L4) — Clojure port.
  ...
  facts (`omop_cdm.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "omop_cdm")
(def tier "L4")
...
(def entity-specs
  [{:entity "Person"    :plural "persons"               :id-prefix "omopcdm_per"
    :fields [:personId :genderConceptId :yearOfBirth :monthOfBirth :dayOfBirth :birthDatetime :raceConceptId :ethnicityConceptId] :required [:yearOfBirth :monthOfBirth]
    :coerce {:personId :int :genderConceptId :int :yearOfBirth :int :monthOfBirth :int :dayOfBirth :int :raceConceptId :int :ethnicityConceptId :int} :refs {}}
   {:entity "Concept"   :plural "concepts"              :id-prefix "omopcdm_con"
    :fields [:conceptId :conceptName :domainId :vocabularyId :conceptClassId :standardConcept :conceptCode :validStartDate :validEndDate :invalidReason] :required [:conceptName :standardConcept]
    :coerce {:conceptId :int}                           :refs {}}
   {:entity "ConditionOccurrence" :plural "conditionoccurrences" :id-prefix "omopcdm_con"
    :fields [:conditionOccurrenceId :personId :conditionConceptId :conditionStartDate :conditionEndDate :visitOccurrenceId] :required [:conditionStartDate :conditionEndDate]
    :coerce {:conditionOccurrenceId :int :personId :int :conditionConceptId :int :visitOccurrenceId :int} :refs {:personId "Person" :visitOccurrenceId "VisitOccurrence"}}
   {:entity "DrugExposure" :plural "drugexposures"      :id-prefix "omopcdm_dru"
    :fields [:drugExposureId :personId :drugConceptId :drugExposureStartDate :quantity :daysSupply] :required [:drugExposureStartDate :quantity]
    :coerce {:drugExposureId :int :personId :int :drugConceptId :int :quantity :float :daysSupply :int} :refs {:personId "Person"}}
   {:entity "VisitOccurrence" :plural "visitoccurrences" :id-prefix "omopcdm_vis"
    :fields [:visitOccurrenceId :personId :visitConceptId :visitStartDate :visitEndDate :providerId] :required [:visitStartDate :visitEndDate]
    :coerce {:visitOccurrenceId :int :personId :int :visitConceptId :int :providerId :int} :refs {:personId "Person"}}
   {:entity "Observation" :plural "observations"        :id-prefix "omopcdm_obs"
    :fields [:observationId :personId :observationConceptId :observationDate :valueAsNumber :valueAsString] :required [:observationDate :valueAsNumber]
    :coerce {:observationId :int :personId :int :observationConceptId :int :valueAsNumber :float} :refs {:personId "Person"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"omop_cdm.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "omop_cdm-compat" :tier tier :entities entities} 200])
...
(defn start! [] :omop_cdm-compat/ready)
